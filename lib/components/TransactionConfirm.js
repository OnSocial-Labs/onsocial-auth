...
export const TransactionConfirm = ({ transaction, onComplete, }) => {
    ...
    return (<View style={sharedStyles.container}>
      ...
      {transaction.actions.map((action, index) => (<Text key={index}>
          {action.type}: {action.params.amount || "N/A"}
        </Text>))}
      {loading ? (<Text>Processing...</Text>) : (<Button title="Confirm" onPress={handleConfirm}/>)}
    </View>);
};
