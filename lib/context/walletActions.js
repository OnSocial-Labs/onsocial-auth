...
export const createNewAccount = async (accountId, network, wallet, setAccountId, setNetwork, setMnemonic, setBalance) => {
    try {
        ...
        const keyPairInstance = KeyPair.fromString(utils.serialize.base_encode(seed.slice(0, 32))); // Updated
...
export const signTransaction = async (tx, wallet, accountId, setBalance) => {
    try {
        ...
        const actions = tx.actions.map((action) => {
            switch (action.type) {
                case "Transfer":
                    return nearAPI.transactions.transfer(nearAPI.utils.format.parseNearAmount(action.params.amount));
                default:
