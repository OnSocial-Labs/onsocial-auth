...
export const WalletSetup: React.FC<WalletSetupProps> = ({
  ...
  const onAuthSetupComplete = (accountId: string) => {
  ...
  return (
    <View style={sharedStyles.container}>
      ...
      <TextInput
        ...
        placeholder={`Enter username (e.g., alice.${network})`}
        value={username}
      ...
      {showImport && (
        <View style={sharedStyles.modalOverlay}>
          <View style={sharedStyles.modalContent}>
            <TextInput
              style={sharedStyles.input}
              placeholder="Paste your 12-word mnemonic"
              value={importKey}
      ...
      <AuthSetup
        visible={showAuthSetup}
        accountId={`${username.toLowerCase()}.${network}`}
        onComplete={onAuthSetupComplete}
      />
    </View>
  );
};
